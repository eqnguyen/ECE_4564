...
# 4a. go to rsync directory in windows cmd prompt
# 4b. run cwrsync.cmd to configure ssh
# 4c. go to the rsync/bin director in windows command prompt
# 4d. run "ssh-keygen.exe -t rsa -C "some text" then press enter 4 times
# 4e. Copy the text from rsync/home/{user}/.ssh/id_rsa.pub to ~/.ssh/authorized_keys on your pi
# 5. You may have to start the rsync daemon on the pi with:
# 		sudo systemctl enable rsync
#       sudo systemctl start rsync
# 6. Run this script (from win cmd prompt you can enter "python rsync.py")
# 
